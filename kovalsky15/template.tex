...
\begin{frame}
 ...
 \visible<2> {
  \TikzDraw {
    ...
    \node [fill=blue!20, draw=white, rounded corners=1mm, text opacity=1, fill opacity=10] at (-0.1, -2.8) {\Large Efficiency \& Scalability};
  }
 }
 %\gridlines
\end{frame}
...
\begin{frame}
 ...
 \TikzDraw {
  ...
  \visible<2-> {
    \node at (3, 0) {\parbox[t]{0.5\textwidth}{
      ...
      \begin{equation*}
      \begin{split}
	&\|T\BOLD{x}-T\BOLD{x}_0\|_2^2=\|T\BOLD{x}-\prod_\mathcal{D}(T\BOLD{x}_0)\|^2_2 \\
	&+\|\prod_\mathcal{D}(T\BOLD{x}_0)-T\BOLD{x}_0\|_2^2
      \end{split}
      \end{equation*}
      \begin{equation*}
       \boxed {
	\begin{aligned}
	 &\BOLD{x}_* = \argmin_\BOLD{x} \|T\BOLD{x}-\prod_\mathcal{D}(T\BOLD{x}_0)\|_2 \\
 ...
 \visible<3> {
  \TikzDraw {
    \draw[red, very thick] (1.8, -2) -- (3.8, -2.6);
    \node at (4, -3.1) {\TODO{Alternating optimization}};
  }
 }
 %\gridlines
\end{frame}
